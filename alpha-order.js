...
const solution = (words) => {
  ...
  if (!words || words.length < 2) {
    console.log('not enough words provided')
    return []
  }
 ...
 */
  ...
  for (let i = 0; i < words.length - 1; i++) {
    ...
    if (!mismatchFound && words[i].length > words[i + 1].length) {
      console.log('the list of words is incorrectly sorted')
      return []
    }
  }
  ...
  if (!singlePath(adjacencyList, order)) {
    console.log(
      'more than one path found, not enough information provided to determine alphabetical sort'
    )
    return []
  }
